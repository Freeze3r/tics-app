    ...
    <div className="rounded-2xl bg-white p-4 dark:bg-ink-800">
      ...
      >
        <div>
          ...
          <div className="text-xs text-ink-800/50 dark:text-sand-100/50">{behavior.sublabel}</div>
        </div>
        <span className="text-ink-800/40 dark:text-sand-100/40">{open ? '−' : '+'}</span>
      </button>

      {open && (
        <div className="mt-4 flex flex-col gap-4">
          <p className="text-sm leading-relaxed text-ink-800/70 dark:text-sand-100/70">
            {behavior.mechanism}
          </p>

          <div>
            <p className="mb-2 text-sm font-semibold text-sage-600 dark:text-sage-400">
            ...
            <div className="flex flex-col gap-2">
              {behavior.barriers.map((item) => {
                ...
                return (
                  <label
                    key={item}
                    className="flex items-center gap-2 text-sm text-ink-800/80 dark:text-sand-100/80"
                  >
                    <input
                      type="checkbox"
                      checked={checked}
  ...
  return (
    <main className="px-6 py-8">
      <div className="mx-auto max-w-md pb-6">
        <h1 className="text-2xl font-bold text-ink-800 dark:text-sand-100">Bibliothèque</h1>
        <p className="mt-1 text-ink-800/60 dark:text-sand-100/60">
          Comprendre ce qui se passe, sans jugement.
        </p>

        <section className="mt-6 rounded-2xl bg-white p-4 dark:bg-ink-800">
          <p className="text-sm font-semibold text-ink-800 dark:text-sand-100">
            Les BFRB, c'est quoi au juste ?
          </p>
          <p className="mt-2 text-sm leading-relaxed text-ink-800/70 dark:text-sand-100/70">
        ...
        <section className="mt-4 rounded-2xl bg-sage-100/60 p-4 dark:bg-sage-700/10">
        ...
        <section className="mt-6 rounded-2xl bg-white p-4 dark:bg-ink-800">
          ...
          <p className="mt-2 text-sm text-ink-800/70 dark:text-sand-100/70">
