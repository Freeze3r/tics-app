  ...
  return (
    <main className="px-6 py-8">
      <div className="mx-auto max-w-md pb-6">
        <h1 className="text-2xl font-bold text-ink-800 dark:text-sand-100">Journal</h1>
        ...
        <section className="mt-6 rounded-2xl bg-white p-5 dark:bg-ink-800">
          <div className="flex justify-between">
            {MOODS.map((m) => (
              ...
              >
                ...
                <span className="text-xs text-ink-800/60 dark:text-sand-100/60">{m.label}</span>
              </button>
            ))}
          </div>
          ...
          <textarea
            ...
            rows={3}
            className="mt-4 w-full resize-none rounded-2xl border-2 border-sage-200 bg-transparent p-3 text-sm text-ink-800 placeholder:text-ink-800/40 focus:border-sage-400 focus:outline-none dark:border-sage-700 dark:text-sand-100 dark:placeholder:text-sand-100/40"
          />
          ...
          <section className="mt-8">
            ...
            <div className="mt-3 flex flex-col gap-2">
              {entries.map((e) => {
                ...
                return (
                  <div key={e.id} className="rounded-2xl bg-white p-4 text-sm dark:bg-ink-800">
                    ...
                    {e.note && (
                      <p className="mt-1 text-ink-800/70 dark:text-sand-100/70">{e.note}</p>
                    )}
                  </div>
                )
              })}
